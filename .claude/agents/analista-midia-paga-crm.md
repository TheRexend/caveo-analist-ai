...
**Mapeamento de variações de `UtmSou__c` por canal:**

| Canal | Valores de UtmSou__c | Padrão SOQL |
|---|---|---|
| **Meta — Facebook** | `facebook`, `Facebook_Feed`, `Facebook_Reels`, `Facebook_Stories`, `Facebook_Marketplace`, `Facebook_Instream_Video`, `Facebook_Search`, `Facebook_Right_Column`, `Facebook_Video_Feed` | `UtmSou__c LIKE 'facebook%'` |
| **Meta — Messenger** | `Messenger_Stories`, `Messenger_Sponsored_Messages`, `Messenger_Inbox` | `UtmSou__c LIKE 'messenger%'` |
| **Meta — Audience Network** | `Audience_Network_Native`, `Audience_Network_Banner`, `Audience_Network_Interstitial`, `Audience_Network_Rewarded_Video` | `UtmSou__c LIKE 'audience_network%'` |
| **Meta — Instagram** | `Instagram_Feed`, `Instagram_Profile_Feed`, `Instagram_Stories`, `Instagram_Reels`, `Instagram_Profile_Reels`, `Instagram_Search` | `UtmSou__c LIKE 'Instagram%'` |
| **Meta — Template não resolvido** | `{{placement}}` | `UtmSou__c = '{{placement}}'` — contabilizar como Meta Ads |
| **Google Ads** | `google`, `Google` (variações de capitalização) | `UtmSou__c LIKE 'google%'` |
| **YouTube** | `Youtube` | `UtmSou__c = 'Youtube'` |
| **Não Digital** | `bioinsta`, `Indicação`, `indicação`, `INDICACAO`, `amigo`, `Amigo`, `Amigos`, `amigos`, `wapp`, `whatsapp`, `WPP`, `faculdade`, `Faculdade`, `Na faculdade`, `Insta`, `Embaixador`, `Internet.`, `Youtube` (orgânico), e quaisquer valores não mapeados acima | negação de todos os canais pagos |

> **Regra de agrupamento:**
> - **Meta Ads** = `facebook%` + `Instagram%` + `messenger%` + `audience_network%` + `{{placement}}`
> - **Google/YouTube** = `google%` + `Youtube`
> - **Não Digital** = todo o resto com UTM preenchido (indicações, boca a boca, faculdade, WhatsApp, etc.)
> - **Sem UTM** = `UtmSou__c = null` — pipeline não atribuído
>
> Sempre apresentar os 4 grupos para garantir que o total de oportunidades no Claude bate com o total no Salesforce.
...
**Query — Leads com UTM por período (todos os canais pagos):**
```sql
SELECT Id, UtmSou__c, UtmMed__c, UtmCam__c, UtmCon__c, UtmTer__c, UrlUtm__c,
       Status, LeadSource, CreatedDate, IsConverted, ConvertedDate
FROM Lead
WHERE CreatedDate >= [DATA_INICIO]T00:00:00Z
  AND CreatedDate <= [DATA_FIM]T23:59:59Z
  AND (UtmSou__c LIKE 'facebook%'
    OR UtmSou__c LIKE 'instagram%'
    OR UtmSou__c LIKE 'Instagram%'
    OR UtmSou__c LIKE 'messenger%'
    OR UtmSou__c LIKE 'audience_network%'
    OR UtmSou__c = 'google'
    OR UtmSou__c = 'Youtube')
ORDER BY CreatedDate DESC
```

**Query — Visão completa de oportunidades por grupo de canal (use sempre para validar totais):**
```sql
...
```
*Após receber o resultado, categorize cada `UtmSou__c` nos grupos: Meta Ads / Google+YouTube / Não Digital / Sem UTM. Some os totais e compare com o total geral de oportunidades do período para confirmar cobertura completa.*

**Query — Oportunidades de mídia paga (Meta + Google + YouTube):**
```sql
...
WHERE CreatedDate >= [DATA_INICIO]T00:00:00Z
  ...
  AND (UtmSou__c LIKE 'facebook%'
    ...
    OR UtmSou__c LIKE 'messenger%'
    OR UtmSou__c LIKE 'audience_network%'
    OR UtmSou__c = '{{placement}}'
    OR UtmSou__c LIKE 'google%'
    OR UtmSou__c = 'Youtube')
ORDER BY CreatedDate DESC
```

**Query — Funil agregado por campanha e canal (Oportunidades):**
```sql
SELECT UtmCam__c, UtmSou__c, StageName, COUNT(Id) total
FROM Opportunity
WHERE UtmCam__c != null
  AND CreatedDate >= [DATA_INICIO]T00:00:00Z
  AND CreatedDate <= [DATA_FIM]T23:59:59Z
GROUP BY UtmCam__c, UtmSou__c, StageName
ORDER BY COUNT(Id) DESC
```

**Query — Apenas Meta Ads (todos os placements + template não resolvido):**
```sql
WHERE (UtmSou__c LIKE 'facebook%'
    OR UtmSou__c LIKE 'Instagram%'
    OR UtmSou__c LIKE 'messenger%'
    OR UtmSou__c LIKE 'audience_network%'
    OR UtmSou__c = '{{placement}}')
...
**Query — Apenas Google Ads + YouTube:**
```sql
WHERE UtmSou__c LIKE 'google%' OR UtmSou__c = 'Youtube'
```

**Query — Não Digital (canais orgânicos e manuais com UTM preenchido):**
```sql
SELECT UtmSou__c, COUNT(Id) total
FROM Opportunity
WHERE CreatedDate >= [DATA_INICIO]T00:00:00Z
  AND CreatedDate <= [DATA_FIM]T23:59:59Z
  AND UtmSou__c != null
  AND (NOT UtmSou__c LIKE 'facebook%')
  AND (NOT UtmSou__c LIKE 'Instagram%')
  AND (NOT UtmSou__c LIKE 'messenger%')
  AND (NOT UtmSou__c LIKE 'audience_network%')
  AND UtmSou__c != '{{placement}}'
  AND (NOT UtmSou__c LIKE 'google%')
  AND UtmSou__c != 'Youtube'
GROUP BY UtmSou__c
ORDER BY COUNT(Id) DESC
```
...
| Agente Conectado | Como se Conecta | Quando Acionar |
|---|---|---|
| **Agente de Criativos** | Recebe o diagnóstico de CTR baixo ou alta taxa de Lead→MQL falhando por um criativo específico (utm_content) | CTR abaixo de benchmark ou MQL baixo concentrado num conjunto de anúncios |
| **Agente de Inside Sales** | Recebe o diagnóstico de SQL alto mas fechamento baixo — indica problema no processo comercial, não em mídia | Taxa SQL→Oportunidade ou Oportunidade→Ganho abaixo de benchmark |
| **Agente de Tracking** | Acionado quando há mais de 15% de leads sem UTM ou inconsistência na nomenclatura de campanhas | Taxa de leads sem UTM > 15% ou utm_campaign divergente entre CRM e plataforma |

> **Lógica de interdependência:** este agente é o ponto central de diagnóstico — recebe dados brutos de mídia e CRM, entrega o diagnóstico integrado, e direciona os casos específicos (criativo, vendas, tracking) para os agentes especializados correspondentes.
