...
Executar **três queries**:

**Query E1 — Oportunidades (todos os canais pagos):**

```sql
SELECT Id, UtmSou__c
FROM Opportunity
WHERE CreatedDate >= [CUR_START]T00:00:00Z
  AND CreatedDate <= [CUR_END]T23:59:59Z
  AND (UtmSou__c LIKE '%google%'
       OR UtmSou__c LIKE '%youtube%'
       OR UtmSou__c LIKE '%Youtube%'
       OR UtmSou__c LIKE '%facebook%'
...
**Query E2 — Fechamentos (mesmo filtro + estágio):**
...
WHERE CreatedDate >= [CUR_START]T00:00:00Z
  AND CreatedDate <= [CUR_END]T23:59:59Z
  AND StageName IN ('Fechado', 'Ganho não Identificado')
  AND (UtmSou__c LIKE '%google%'
       OR UtmSou__c LIKE '%youtube%'
       OR UtmSou__c LIKE '%Youtube%'
       OR UtmSou__c LIKE '%facebook%'
...
```sql
SELECT StageName, UtmSou__c, COUNT(Id) total
FROM Opportunity
WHERE CreatedDate >= [CUR_START]T00:00:00Z
  AND CreatedDate <= [CUR_END]T23:59:59Z
  AND (UtmSou__c LIKE '%google%'
...
| Variável | Fonte |
|---|---|
| `google_cur_oportunidades` | COUNT de E1 onde `UtmSou__c LIKE '%google%' OR LIKE '%youtube%' OR LIKE '%Youtube%'` |
| `google_cur_fechamentos` | COUNT de E2 com mesmo filtro Google |
| `meta_cur_oportunidades` | COUNT de E1 onde `UtmSou__c LIKE '%facebook%' OR '%instagram%' OR '%Instagram%' OR '%messenger%' OR '%audience_network%' OR = '{{placement}}'` |
| `meta_cur_fechamentos` | COUNT de E2 com mesmo filtro Meta |
| `sf_cur_funil_google` | Linhas de E3 com UTM Google — distribuição de estágios |
| `sf_cur_funil_meta` | Linhas de E3 com UTM Meta — distribuição de estágios |
...
- **StageName Salesforce:** estágios exatos são `'Fechado'` e `'Ganho não Identificado'` (com ã e I maiúsculo). Confirmado em produção em 2026-05-22.
- **UTM casing:** incluir variantes com maiúscula (`Instagram`, `Youtube`) nas queries SOQL para não perder registros.
