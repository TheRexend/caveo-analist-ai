...
**Query A — Todas as Oportunidades do período (Meta + Google):**

```sql
SELECT Id, Name, StageName, Amount, CreatedDate, UtmSou__c, UtmCam__c
FROM Opportunity
...
**Query B — Somente Fechamentos (mesmo filtro de canal + estágio):**
...
| Variável | Cálculo |
|---|---|
| `sf_meta_oportunidades` | COUNT de Query A onde `UtmSou__c LIKE '%facebook%' OR '%instagram%' OR '%messenger%' OR '%audience_network%'` |
| `sf_meta_fechamentos` | COUNT de Query B com mesmo filtro Meta |
| `sf_google_oportunidades` | COUNT de Query A onde `UtmSou__c LIKE '%google%' OR '%youtube%'` |
| `sf_google_fechamentos` | COUNT de Query B com mesmo filtro Google |
...
- **Fechamentos Salesforce:** estágios exatos são `Fechado` e `Ganho não Identificado` (com I maiúsculo). Confirmado em produção em 2026-05-22.
