...
- **Filtro de canal pago** (`UtmSou__c`): `facebook%` / `Instagram%` / `messenger%` /
  `audience_network%` / `{{placement}}` / `google%` / `Youtube`.
...
WHERE CreatedDate >= [DATA_INICIO]T00:00:00Z
  ...
  AND (UtmSou__c LIKE 'facebook%' OR UtmSou__c LIKE 'Instagram%'
    OR UtmSou__c LIKE 'messenger%' OR UtmSou__c LIKE 'audience_network%'
    OR UtmSou__c = '{{placement}}' OR UtmSou__c LIKE 'google%'
