...
def test_cell_updates_mm_usa_linhas_base_sem_offset():
    ups = dict(cell_updates("mm", {"invest_meta": 1000.0, "leads_google": 37}, {}))
    assert ups["B30"] == 1000.0
    assert ups["F36"] == 37


def test_cell_updates_rf_soma_offset_28():
    ups = dict(cell_updates("rf", {"invest_meta": 500.0, "leads_google": 10}, {}))
...
def test_cell_updates_ignora_none_mas_mantem_zero_explicito():
    ups = dict(cell_updates("mm", {"invest_meta": None, "mql_meta": 0}, {}))
    assert "B30" not in ups
    assert ups["B38"] == 0


def test_cell_updates_grava_todos_os_estagios_com_zero_explicito():
    ups = dict(cell_updates("mm", {}, {"meta": {"Fechado Ganho": 4}}))
...
def test_cell_updates_estagios_rf_usa_offset():
    ups = dict(cell_updates("rf", {}, {"google": {"Nova": 5}}))
    assert ups["F79"] == 5  # 51 (Nova, base MM) + 28
