...
O contrato central é COLS: ele não contém as colunas de rótulo (B, J, R, U) nem
as de fórmula (V..AF), então é estruturalmente impossível esta skill sobrescrever
...
# NÃO inclui B, J, R, U (rótulos) nem V..AF (fórmulas) — de propósito.
#
# Cinco blocos, na ordem da aba: Meta Ads (plataforma C..F + Salesforce G..I),
# Google Ads (plataforma K..N + Salesforce O..Q) e GA4 (S..T).
# As seis colunas de Salesforce (G/H/I e O/P/Q) entraram em 2026-08-18.
# ATENÇÃO: dentro do bloco Meta o investimento é a coluna F — NÃO a I, que é
# fechamentos. O bloco Google inverte essa ordem (investimento N vem ANTES do
# Salesforce O..Q); os dois blocos não são simétricos, é assim na aba mesmo.
COLS = {
    "meta_impressoes": "C",
    "meta_cliques": "D",
    "meta_leads": "E",
...
# Formato da coluna do bounce. Copiado da linha 2, a única que já vinha
# formatada à mão. Ver ensure_bounce_format().
BOUNCE_COL = "T"
BOUNCE_FORMAT = {"numberFormat": {"type": "PERCENT", "pattern": "0.00%"}}

# grid_row é a fatia C..T devolvida pelo gspread: índice 0 = C.
# ord() só funciona porque C..T são todas colunas de UMA letra; as de fórmula
# (V..AF) nunca passam por aqui, só por formatação de string em ROW_FORMULAS.
_GRID_FIRST_COL = "C"
...
def check_consecutive(date_row_map):
    ...
    as linhas seguintes e quebraria as âncoras das fórmulas V..AF."""
    if not date_row_map:
        return []
...
def _filled_count(grid_row):
    """Quantas das dezesseis células graváveis estão preenchidas nessa fatia C..T.

    Olha só as colunas de COLS: J e R carregam rótulo fixo em toda linha e não
    podem contar como 'dado'. O gspread também corta os vazios do fim, então a
    fatia pode vir mais curta que 18 — daí o teste de comprimento."""
    count = 0
...
def row_is_empty(grid_row):
    """True se nenhuma das dezesseis células graváveis estiver preenchida."""
...
def append_rows_payload(first_new_row, dates):
    ...
    first_new_row: data na coluna A, rótulos em B/J/R/U e as onze fórmulas do
...
def cell_updates(row, metrics):
    ...
    Aceitar um subconjunto das chaves é o que permite o backfill cirúrgico das
    seis colunas de Salesforce (G/H/I e O/P/Q) numa linha que já tem
    Meta/Google/GA4 gravados."""
...
def ensure_bounce_format(worksheet, first_row, last_row):
    """Garante que a coluna do bounce esteja formatada como % no intervalo.
    ...
    if last_row < first_row:
        return None
    cell_range = f"{BOUNCE_COL}{first_row}:{BOUNCE_COL}{last_row}"
