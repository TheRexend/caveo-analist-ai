...
function TimelineChartBase({ days, platform }: { days: TimelineDay[]; platform: Platform }) {
  ...
  return (
    <div className="timeline-card">
      ...
      <div className="chart-wrap" role="img" aria-label="Gráfico de evolução temporal das métricas de mídia paga">
      ...
      <div className="legend-chips" style={{ marginTop: 12 }}>
        {ORDER.map((k) => (
          <button key={k} className={`legend-chip ${active[k] ? "active" : ""}`} aria-pressed={active[k]} onClick={() => setActive((a) => ({ ...a, [k]: !a[k] }))}>
...
export const TimelineChart = memo(TimelineChartBase);
